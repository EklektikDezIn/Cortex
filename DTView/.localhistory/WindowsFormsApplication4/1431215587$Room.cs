    ...
    class Room
    {
        ...
        public List<Vector3>[] pathFinder(Vector3 alpha, Vector3 omega)
        {
            ...
            while (!alpha.Equals(omega))
            {
                ...
                {
                    min = Math.Abs(data[0].X);
                }
                Vector3 jump = new Vector3((float)Math.Truncate(distance.X / Math.Abs(data[0].X)), (float)Math.Truncate(distance.Y / Math.Abs(data[0].X)), (float)Math.Truncate(distance.Z / Math.Abs(data[0].X)));


                //"Don't do it wrong"-Prim

                distance = alpha.distanceTo(home.offSet(jump.offSetX(j, j, j)));
                while (!distance.Equals(voids))
                {
                    if (distance.X == 0)
                    {
                        xcount = 0;
