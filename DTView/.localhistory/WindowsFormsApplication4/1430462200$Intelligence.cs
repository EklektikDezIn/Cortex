...
namespace NextGen
{
    class Intelligence
    {
                    ...
                    for (int x = 0; x < Inpt.getXDim(); x++)
                    {
                        for (int y = 0; y < Inpt.getYDim(); y++)
                        {
                            for (int z = 0; z < Inpt.getZDim(); z++)
                            {
                                ...
                                val[j] += Logos(option, alpha, journey);
                            }
                        }
                    }
        ...
        public int Logos(Vector3 home, Vector3 target, Vector4 path)
        {
            char thing = Inpt.getObjectAt(path.Xyz);
            ...
            int y = (int)path.Y - (int)target.Y;
            double energy = path.W + Math.Cos(Math.Atan(y / path.W));
