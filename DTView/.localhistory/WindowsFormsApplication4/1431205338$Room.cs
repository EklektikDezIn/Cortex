...
namespace NextGen
{
    class Room
    {
        ...
        public List<Vector3>[] pathFinder(Vector3 alpha, Vector3 omega)
        {
            ...
            Vector3 jump = new Vector3((float)Math.Floor(omega.X / data[0].X), (float)Math.Floor(omega.Y / data[0].X), (float)Math.Floor(omega.Z / data[0].X));
            int xcount;
            int ycount;
            int zcount;
            for (int i = 0; i < 6; i++)
            ...
            while (!distance.Equals(new Vector3(0, 0, 0)))
            {
                if (distance.X == 0)
                {
                    xcount = 0;
                ...
                distance = alpha.distanceTo(omega);
            }
            return Paths;
        }
