    ...
    class Intelligence
    {
            ...
            for (int x = 0; x < options.getXDim(); x++)
            {
                for (int y = 0; y < options.getYDim(); y++)
                {
                    for (int z = 0; z < options.getZDim(); z++)
                    {
                        ...
                        if (options.Type(alpha, 'D') >=7 && options.Type(alpha.offSet(0, 1, 0), 'D') <=1)
                        {
        ...
        public List<Vector3> oneStep(Vector3 start, List<Vector4> beenThere)
        {
            ...
            foreach (Vector3 pos in Choices)
                foreach(Vector4 comp in beenThere)
            {
                if (start.X == pos.X && start.Z + 1 == pos.Z && pos.Y - start.Y < 1 && start.offSet(0, 0, 1).Equals(comp)) { data.Add(new Vector3(comp).offSet(0, 0, 1)); }
                if (start.X == pos.X && start.Z - 1 == pos.Z && pos.Y - start.Y < 1 && start.offSet(0, 0, -1).Equals(comp)) { data.Add(new Vector3(comp).offSet(0, 0, -1)); }
                if (start.X + 1 == pos.X && start.Z == pos.Z && pos.Y - start.Y < 1 && start.offSet(1, 0, 0).Equals(comp)) { data.Add(new Vector3(comp).offSet(1, 0, 0)); }
                if (start.X - 1 == pos.X && start.Z == pos.Z && pos.Y - start.Y < 1 && start.offSet(-1, 0, 0).Equals(comp)) { data.Add(new Vector3(comp).offSet(-1, 0, 0)); }
            }
            return data;
        }
