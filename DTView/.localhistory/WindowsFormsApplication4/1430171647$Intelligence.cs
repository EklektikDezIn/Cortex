...
namespace NextGen
{
    class Intelligence
    {
        ...
        public List<Vector3> location(Room options)
        {
            ...
            Room options = Inpt.getObjectsInRadius(me, stat[3]);
            for (int x = 0; x < options.getXDim(); x++)
            {
                for (int y = 0; y < options.getYDim(); y++)
                {
                    for (int z = 0; z < options.getZDim(); z++)
                    {
                        ...
                        if (Inpt.Type(alpha, 'D') == 0 && Inpt.Type(alpha.offSet(0, 1, 0), 'D') == 10)
                        {
        ...
        public List<Vector3> oneStep(Vector3 start)
        {
            List<Vector3> data = new List<Vector3>();
            List<Vector3> Choices = location(Inpt);
            foreach (Vector3 pos in Choices)
            {
                if (start.X == pos.X && start.Z + 1 == pos.Z && pos.Y - start.Y < 1 && !data.Contains(start.offSet(0,0,1))) { data.Add(start.offSet(0, 0, 1)); }
                if (start.X == pos.X && start.Z - 1 == pos.Z && pos.Y - start.Y < 1 && !data.Contains(start.offSet(0, 0, -1))) { data.Add(start.offSet(0, 0, -1)); }
                if (start.X + 1 == pos.X && start.Z == pos.Z && pos.Y - start.Y < 1 && !data.Contains(start.offSet(1, 0, 0))) { data.Add(start.offSet(1, 0, 0)); }
                if (start.X - 1 == pos.X && start.Z == pos.Z && pos.Y - start.Y < 1 && !data.Contains(start.offSet(-1, 0, 0))) { data.Add(start.offSet(-1, 0, 0)); }
            }
            return data;
        }
