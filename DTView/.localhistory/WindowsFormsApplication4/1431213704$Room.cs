    ...
    class Room
    {
        ...
        public List<Vector3>[] pathFinder(Vector3 alpha, Vector3 omega)
        {
            ...
            while (!alpha.Equals(omega))
            {
                ...
                Vector3 jump = new Vector3((float)Math.Floor(distance.X / Math.Abs(data[0].X)), (float)Math.Floor(distance.Y / Math.Abs(data[0].X)), (float)Math.Floor(distance.Z / Math.Abs(data[0].X)));


                //"Don't do it wrong"-Prim
                Vector3 voids = new Vector3(0, 0, 0);
                while (!alpha.distanceTo(home.offSet(jump.offSetX(j, j, j))).Equals(voids))
                {
                    Vector3 temp = alpha.distanceTo(home.offSet(jump.offSetX(j, j, j)));
                    if (distance.X == 0)
                    {
                        xcount = 0;
                    }
