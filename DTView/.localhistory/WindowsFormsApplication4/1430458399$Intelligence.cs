...
namespace NextGen
{
    class Intelligence
    {
        ...
        public List<Vector3> location(Room options, List<Vector3> Targets)
        {
        ...
        public List<Vector4> paths(Vector3 coor, Vector3 Targets)
        {
            int i = 0;
        ...
        public List<Vector3> oneStep(Vector3 start, List<Vector4> beenThere, List<Vector3> Targets)
        {
            ...
            List<Vector3> Choices = location(Inpt, Targets);
        ...
        public Vector3 Psyche()
        {
            ...
            {
                List<Vector4> Path = paths(option, Targets);
                foreach (Vector4 journey in Path)
                {
                    Logos(option, journey);
                }
        ...
        public int Logos(Vector3 alpha, Vector4 beta)
        {
            char thing = Inpt.getObjectAt(beta.Xyz);
            Archive data = Main.Archives[0];
            foreach (Archive archie in Main.Archives)
            {
                if (archie.Symbol.Equals(thing))
                {
                    data = archie.Clone();
                }
            }

            int y = (int)beta.Y - (int)alpha.Y;
            double energy = beta.W + Math.Cos(Math.Atan(y / beta.W));

            double Value = 0;
            Value += (500 - stat[4]) * data.Benefits[2];//health
            Value += stat[6] * data.Benefits[0];//food
            Value += stat[7] * data.Benefits[1];//water
        ...
        public int genVal(char Object, Vector3 alpha, Vector3 beta, int[] Stats)
        {
            return -1;
            //
        }
