...
namespace NextGen
{
    class Intelligence
    {
        ...
        public List<Vector3> oneStep(Vector3 start, List<Vector4> beenThere)
        {
            ...
            {
                if (start.X == pos.X && start.Z + 1 == pos.Z && pos.Y - start.Y < 1 && !comp.Contains(start.offSet(0, 0, 1))) { data.Add(start.offSet(0, 0, 1)); }
                if (start.X == pos.X && start.Z - 1 == pos.Z && pos.Y - start.Y < 1 && !comp.Contains(start.offSet(0, 0, -1))) { data.Add(start.offSet(0, 0, -1)); }
                if (start.X + 1 == pos.X && start.Z == pos.Z && pos.Y - start.Y < 1 && !comp.Contains(start.offSet(1, 0, 0))) { data.Add(start.offSet(1, 0, 0)); }
                if (start.X - 1 == pos.X && start.Z == pos.Z && pos.Y - start.Y < 1 && !comp.Contains(start.offSet(-1, 0, 0))) { data.Add(start.offSet(-1, 0, 0)); }
            }
            return data;
        }
