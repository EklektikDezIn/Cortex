...
namespace NextGen
{
    class Intelligence
    {
        ...
        public List<Vector3> oneStep(Vector3 start, List<Vector4> beenThere)
        {
            ...
            {
                if (start.X == pos.X && start.Z + 1 == pos.Z && pos.Y - start.Y <= 1 && !comp.Contains(new Vector2 (start.X,start.Z)) { data.Add(pos); }
                if (start.X == pos.X && start.Z - 1 == pos.Z && pos.Y - start.Y <= 1 && !comp.Contains(start.Xz)) { data.Add(pos); }
                if (start.X + 1 == pos.X && start.Z == pos.Z && pos.Y - start.Y <= 1 && !comp.Contains(start.Xz)) { data.Add(pos); }
                if (start.X - 1 == pos.X && start.Z == pos.Z && pos.Y - start.Y <= 1 && !comp.Contains(start.Xz)) { data.Add(pos); }
            }
            return data;
        }
