...
namespace NextGen
{
    class Intelligence
    {
            ...
            for (int x = 0; x < options.getXDim(); x++)
            {
                for (int y = 0; y < options.getYDim(); y++)
                {
                    for (int z = 0; z < options.getZDim(); z++)
                    {
                        ...
                        if (Inpt.Type(alpha, 'D') >=7 && Inpt.Type(alpha.offSet(0, 1, 0), 'D') <=1)
                        {
        ...
        public List<Vector4> paths(Vector3 coor)
        {
            ...
            possible.AddRange(oneStep(me, data));
        ...
        public List<Vector3> oneStep(Vector3 start, List<Vector4> beenThere)
        {
            ...
            {
                if (start.X == pos.X && start.Z + 1 == pos.Z && pos.Y - start.Y < 1 && comp.start.offSet(0, 0, 1).Equals(comp)) { data.Add(pos); }
                if (start.X == pos.X && start.Z - 1 == pos.Z && pos.Y - start.Y < 1 && start.offSet(0, 0, -1).Equals(comp)) { data.Add(pos); }
                if (start.X + 1 == pos.X && start.Z == pos.Z && pos.Y - start.Y < 1 && start.offSet(1, 0, 0).Equals(comp)) { data.Add(pos); }
                if (start.X - 1 == pos.X && start.Z == pos.Z && pos.Y - start.Y < 1 && start.offSet(-1, 0, 0).Equals(comp)) { data.Add(pos); }
            }
            return data;
        }
